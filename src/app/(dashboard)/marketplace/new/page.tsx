  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch('/api/listings', {
        ...
        body: JSON.stringify({ title, description, price, category, condition, listing_type: listingType, images: imageUrl ? [imageUrl] : [] }),
      })
      if (res.ok) {
      ...
      >
        ...
        <div style={{ marginBottom: '28px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            {([['sell', '🛍️ Sell Item', 'List for purchase'], ['borrow', '🤝 Lend for Borrowing', 'Let others borrow']] as const).map(([type, title, sub]) => (
              <button
                key={type}
                onClick={() => setListingType(type)}
                style={{
                  flex: 1, padding: '14px 16px', borderRadius: '14px', cursor: 'pointer',
                  border: '2px solid', textAlign: 'left', transition: 'all 0.2s',
                  ...(listingType === type
              ...
              >
                <p style={{ fontWeight: 700, fontSize: '14px', marginBottom: '2px' }}>{title}</p>
        ...
        >
          ...
          {uploading ? (
            <>
              <div style={{ width: '48px', height: '48px', borderRadius: '14px', backgroundColor: 'var(--surface-container)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
                <Upload size={22} style={{ color: 'var(--primary)', animation: 'pulse 1s infinite' }} />
              </div>
              <p style={{ fontSize: '14px', fontWeight: 600, color: 'var(--primary)' }}>Uploading...</p>
            </>
          ) : imageUrl ? (
            <>
              <img
              ...
              />
              <p style={{ fontSize: '12px', color: 'var(--on-surface-variant)' }}>Click to replace image</p>
            </>
          ) : (
            <>
              <div style={{ width: '48px', height: '48px', borderRadius: '14px', backgroundColor: 'var(--surface-container)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}>
